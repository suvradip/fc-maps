        ...
        data = page.evaluate(function() {
          ...
          return prep;
             ...
             if( data.length > 0 )
             {
              ...
              var mapTemplate = fs.read("map-md-template.txt");
              mapTemplate = mapTemplate.replace("###mapName###", mapName);
              mapTemplate = mapTemplate.replace("###data###", detailsContent);

              var mapMdFile = fs.read("maps/spec-sheets/"+ mapName +".md");
              mapMdFile = mapMdFile.replace("### List of Entities", mapTemplate);
              fs.write("spec-sheets-maps/" + mapName + ".html", mapMdFile);
              
              console.log("saved at spec-sheets-maps" + mapName + ".html");
